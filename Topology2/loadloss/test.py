#test.py to generate predicted results.
#Takes inputs from different contengencies datasets and predicts an output value which is the loadloss value

import csv
import numpy as np
from keras.models import model_from_json
import time
csvdata = open("result_(n-9).csv", "w")
...
def main():
    time1= time.time()
    with open('val_(n-9).csv') as File:
...
float(row[20]), float(row[21])]
    ...
    #clocking the time taken by model for prediction
    time2=time.time()
    t_time = time2 -time1
